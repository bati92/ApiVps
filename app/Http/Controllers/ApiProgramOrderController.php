...
{   protected $profitService;
            ...
            if($user->balance>=$order->price)
             {
                ...
                $user->save();
                $order->status="مقبول";
                $this->profitService->calculateProfit($order, Program::class);
                $order->save();
   ...
   /* public function calculateProfit($order)
    { $calculatedProfit=0;
       $user=Auth::user();
       $service=Program::findOrFail($order->program_id);
       ...
       $order1 = ProgramOrder::find($order->id); // الحصول على الطلب من نموذج Program
